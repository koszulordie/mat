...
Given an $n\times n$ matrix $A$, we can define different concepts associated to it:
...
If we have two matrices $A\in \mathbb{R}^{m\times n}$ and $B\in \mathbb{R}^{k\times m}$ associated to two linear transformations, then $C=BA\in \mathbb{R}^{k\times n}$ encodes the linear transformation obtained when applying first the transformation of $A$ and then the transformation of $B$. 
...
6. In the exercise 3, is $A(BC)$ equal to $(AB)C$? 
...
<!-- **Exercise**: Create a vector from 1 to 100 (included), and then filter it to keep only the multiples of three. You're not allowed to use the `seq()` function. -->
